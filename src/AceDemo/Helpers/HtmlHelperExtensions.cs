...
namespace AceDemo.Helpers
{
    public static class HtmlHelperExtensions
    {
        ...
        public static IHtmlString CopyRight(this HtmlHelper helper)
        {
            ...
            {
                return helper.Raw(_copyRight);
            }

            var copyRight = Settings.CopyRight;
            if (!string.IsNullOrWhiteSpace(copyRight))
            {
                _copyRight = copyRight.Trim();
            }
            return helper.Raw(_copyRight);
        }
        ...
        /// <param name="partialViewName">The name of the partial view.</param>
        /// <param name="appSetting">The key value of the entry point to look for.</param>
        public static IHtmlString PartialIf(this HtmlHelper htmlHelper, string partialViewName, string appSetting)
        {
            var setting = Settings.GetValue<bool>(appSetting);

            return htmlHelper.PartialIf(partialViewName, setting);
        }
        ...
        {
            var projectVersion = Settings.ProjectVersion;
            if (!string.IsNullOrWhiteSpace(projectVersion))
            {
                _displayVersion = string.Format("{1} {0}", projectVersion, Settings.Project).Trim();
                return;
            }
