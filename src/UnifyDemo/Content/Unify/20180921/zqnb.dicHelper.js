            ...
            return {
                ...
                getCategory: function (categoryCode) {
                    if (categoryCode === undefined || categoryCode === null) {
                        ...
                        throw { name: 'categoryCode should not null or undefined: ' + categoryCode };
                    }
        ...
        createDicCatalogVm = function (config) {
            ...
            var vm = {
                ...
                getCategoryItems: function (theQueryCode, categoryCode, withEmptyItem) {
                    var theVm = this;
                    if (!theQueryCode) {
                        throwException("theQueryCode should not null");
                    }
                    if (!categoryCode) {
                        throwException("categoryCode should not null");
                    }
                ...
                getPhases: function (orgTypeCode, orgId) {
                    //建议直接只是用getCategoryItems；为了保持兼容保留
                    return getCategoryItems({ orgType: orgTypeCode, org: orgId }, knownCategoryCodes.phase);
                },
                getPhasesByOrgId: function (orgId) {
                    //建议直接只是用getCategoryItems；为了保持兼容保留
                    return getCategoryItems({ org: orgId }, knownCategoryCodes.phase);
                },
                getSubjects: function (orgTypeCode, phaseCode) {
                    //建议直接只是用getCategoryItems；为了保持兼容保留
                    return getCategoryItems({ orgType: orgTypeCode, phase: phaseCode }, knownCategoryCodes.subject);
                },
                getGrades: function (orgTypeCode, phaseCode) {
                    //建议直接只是用getCategoryItems；为了保持兼容保留
                    return getCategoryItems({ orgType: orgTypeCode, phase: phaseCode }, knownCategoryCodes.grade);
                }
            };
